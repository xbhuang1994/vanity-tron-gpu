...
printResult(cl_ulong4 seed, cl_ulong round, result r, cl_uchar score,
  ...
  std::cout << strVT100ClearLine << "  Time: " << std::setw(5) << seconds
            << "s Score: " << std::setw(2) << (int)score << " Private: 0x"
            << strPrivate << ' ';
...
Dispatcher::Device::Device(Dispatcher &parent, cl_context &clContext,
    ...
    : m_parent(parent), m_index(index), m_clDeviceId(clDeviceId),
      ...
      m_kernelTransform(mode.transformKernel() == ""
                            ? NULL
                            : createKernel(clProgram, mode.transformKernel())),
      ...
      m_memData1(clContext, m_clQueue,
                 CL_MEM_READ_ONLY | CL_MEM_HOST_WRITE_ONLY, 20 * 100),
...
Dispatcher::Dispatcher(cl_context &clContext, cl_program &clProgram,
    ...
    : m_clContext(clContext), m_clProgram(clProgram), m_mode(mode),
      ...
      m_clScoreQuit(clScoreQuit), m_eventFinished(NULL), m_countPrint(0) {}
...
void Dispatcher::run() {
  ...
  std::cout << "  Always verify that a private key generated by this program "
               "corresponds to the"
            << std::endl;
  std::cout << "  public key printed by importing it to a wallet of your "
               "choice. This program"
            << std::endl;
  std::cout << "  like any software might contain bugs and it does by design "
               "cut corners to"
            << std::endl;
...
void Dispatcher::init() {
  ...
  std::cout << "  This should take less than a minute. The number of objects "
               "initialized on each"
            << std::endl;
  std::cout << "  device is equal to inverse-size * inverse-multiple. To lower"
            << std::endl;
  std::cout
      << "  initialization time (and memory footprint) I suggest lowering the"
      << std::endl;
  std::cout << "  inverse-multiple first. You can do this via the -I switch. "
               "Do note that"
            << std::endl;
...
void Dispatcher::initBegin(Device &d) {
  ...
  CLMemory<cl_uchar>::setKernelArg(d.m_kernelScore, 4,
                                   d.m_clScoreMax); // Updated in handleResult()
...
void Dispatcher::initContinue(Device &d) {
  ...
  if (sizeLeft) {
    ...
    const std::string strOutput =
        "  GPU" + toString(d.m_index) + " initialized";
...
std::string::size_type base58ValueNoException(char c) {
...
void Dispatcher::changeMemData(Device &d) {
  const std::string strHex = "./matching2.txt";
  ...
  for (auto j = 0; j < 100; j++) {
    ...
    for (size_t i = 0; i < strHex2.size(); i += 1) {
      ...
      d.m_memData1[i] = indexStr < 58 ? 255 : 0;
      d.m_memData2[i] = (cl_uchar)strHex2[i];
      // printf("mask %u val
